...
#####rawdata
rawdata <- read.csv("C:/Users/Administrator/Desktop/18년도 종단분석 연구/uga_raw_trans_dm5.csv",head=T)
...
test.feature.var.sh <- test.data.sh[,-10]
test.class.var.sh <- test.data.sh[,10]
...
formula.init <-"최종결과~전형+고교유형+수학내신+과학내신+전체내신+졸업구분+면접유형+면접과목"
...
confusionMatrix(data=nnet.pred.nh, reference=test.class.var.nh, positive='0')
...
importance.nnet.nh1 <- read.csv("C:/Users/Administrator/Desktop/18년도 종단분석 연구/importance.nnet.nh1.csv",head=T)

ggplot(data=importance.nnet.nh1, aes(x=reorder(X,overall),y=overall)) +
  geom_bar(stat="identity") + coord_flip() +
  ggtitle("일반고 출신 지원자 합격을 결정하는 변수별 중요도") +
  ylab('중요도')+
  xlab('')
...
nnet.predict.nh <- predict(nnet.nh,newdata=new.nh)
...
confusionMatrix(data=nnet.pred.sh, reference=test.class.var.sh, positive='0')
...
importance.nnet.sh1 <- read.csv("C:/Users/Administrator/Desktop/18년도 종단분석 연구/importance.nnet.sh1.csv",head=T)

ggplot(data=importance.nnet.sh1, aes(x=reorder(X,overall),y=overall)) +
  geom_bar(stat="identity") + coord_flip() +
  ggtitle("과학고 출신 지원자 합격을 결정하는 변수별 중요도")+
  ylab('중요도')+
  xlab('')
...
write.csv(nnet.predict.nh,"C:/Users/Administrator/Desktop/18년도 종단분석 연구/rawdata.nh.csv")
write.csv(rawdata.nh,"C:/Users/Administrator/Desktop/18년도 종단분석 연구/rawdata.nh.csv")